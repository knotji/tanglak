...
export class GenericCreditCardPDFParser implements ImportParser {
  ...
  async parse(fileData: Buffer | string): Promise<ParseResult> {
    ...
    const systemPrompt = `You are a professional credit card statement parser. Extract all transaction items from the uploaded statement PDF/image document. 
Return ONLY a strict JSON object with: 
- sourceType: "credit_card_statement"
- sourceName: (e.g. "KTC Credit Card Statement")
...
- direction: string ("credit" for payments/refunds, "debit" for charges/purchases)
...
Do not add markdown formatting code block backticks. Return raw JSON text only.`;
    ...
    const response = await fetch(url, {
      ...
      body: JSON.stringify({
        contents: [
          {
            parts: [
              { text: "Extract the transactions from this credit card statement PDF." },
              {
                inline_data: {
    ...
    try {
      ...
      return {
        sourceType: result.sourceType || "credit_card_statement",
        sourceName: result.sourceName || "PDF Credit Card Statement",
        rows: result.rows || [],
  ...
  private getMockResult(): ParseResult {
    ...
    return {
      sourceType: "credit_card_statement",
      sourceName: "Mock PDF Credit Card Statement",
      rows,
      period: {
